    ...
    <?php if (have_rows('visa_category')): ?>
        <?php while (have_rows('visa_category')): the_row(); ?>
            <h5 class="visa-small-title">
                <?php 
                // Get the SVG icon from ACF
                $svg_icon = get_sub_field('svg_icon'); 
                if ($svg_icon): 
                    // Get the SVG content
                    $svg_content = file_get_contents($svg_icon); 
                    echo $svg_content; // Output the SVG code
                endif; 
                ?>
                                ...
                                <div class="visa-content col-7"> <!-- 50% of the card width -->
                                    ...
                                    <div class="d-flex card-bottom-layout">
                                        <div class="container link-container">
                                            <!-- Link to the SVG icon -->
                                            <?php $visa_card_link = get_sub_field('visa_card_link'); ?>
                                            <a href="<?php echo esc_url($visa_card_link); ?>" class="svg-link">
